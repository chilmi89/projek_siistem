...
<body>
                                ...
                                <form id="siswaForm" action="{{ route('siswa.store') }}" method="POST">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <input type="number" class="form-control" id="no_absen" name="no_absen" placeholder="Masukkan nomor absen Anda" required />
                                        </div>
                                    </div>
                                    <div class="d-grid">
    ...
    <script>
        document.getElementById('siswaForm').addEventListener('submit', function(event) {
            ...
            // Simulasikan pengiriman form (Anda bisa mengganti ini dengan AJAX request)
            // Misalnya, kita anggap form berhasil dikirim
            let isSuccess = true; // Ganti dengan logika Anda sendiri
            ...
            if (isSuccess) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // Redirect atau lakukan sesuatu setelah alert ditutup
                        window.location.href = "{{ route('siswa.index') }}"; // Ganti dengan URL yang sesuai
                    }
                });
            } else {
                Swal.fire({
                    icon: 'error',
                    title: 'Gagal!',
                    text: 'Terjadi kesalahan saat mengirim data siswa.',
                    confirmButtonText: 'OK'
                });
            }
        });
